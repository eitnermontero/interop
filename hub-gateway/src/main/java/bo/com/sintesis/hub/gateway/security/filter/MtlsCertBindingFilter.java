 ...
 *   <li>Compara los thumbprints. Si no coinciden → 401 {@code AUTHENTICATION_REQUIRED}
 *       en formato {@link bo.com.sintesis.hub.gateway.web.rest.ApiResponse} (ADR-0005 §6.1.b).</li>
 *   <li>Propaga {@code X-Partner-Id} usando el claim {@code azp} (client_id legible) y
 *       solo como fallback el claim {@code sub}.</li>
 * </ol>
 *
...
public class MtlsCertBindingFilter implements WebFilter, Ordered {
    ...
    public MtlsCertBindingFilter(Environment env, ApiResponseWriter apiResponseWriter) {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        // Usar azp (client_id legible) como partner-id; sub como fallback
        String partnerId = resolverPartnerId(claims);
        ...
        if (cnfThumbprint == null) {
            ...
            return rechazar(exchange, "JWT sin claim cnf.x5t#S256 requerido por RFC 8705.");
        }
        ...
        if (presentedThumbprint == null) {
            ...
            return rechazar(exchange, "No se pudo obtener el certificado de cliente.");
        }
        ...
        if (!cnfThumbprint.equalsIgnoreCase(presentedThumbprint)) {
            ...
            return rechazar(exchange, "El certificado presentado no coincide con el binding del token (RFC 8705).");
        }
    ...
    /**
     * Resuelve el identificador de partner del JWT.
     *
     * <p>Usa el claim {@code azp} (authorized party / client_id legible asignado
     * por Keycloak) como fuente principal. Si {@code azp} no está presente o es
     * vacío, cae al claim {@code sub} como fallback. El {@code azp} es más legible
     * que el {@code sub} (que en Keycloak puede ser un UUID) y es el client_id
     * registrado en el realm {@code hub-partner}.
     *
     * @param claims claims del JWT ya verificado
     * @return identificador del partner, o {@code null} si ningún claim está disponible
     */
    private String resolverPartnerId(Map<String, Object> claims) {
    ...
    /**
     * Rechaza la petición con {@code 401 AUTHENTICATION_REQUIRED} en formato
     * {@link bo.com.sintesis.hub.gateway.web.rest.ApiResponse} (ADR-0005 §6.1.b).
     *
     * @param exchange el exchange reactivo
     * @param detail   detalle técnico del motivo de rechazo
     */
    private Mono<Void> rechazar(ServerWebExchange exchange, String detail) {
        ApiError apiError = new ApiError(CODE_AUTH_REQUIRED, detail, null);
        return apiResponseWriter.writeError(exchange, HttpStatus.UNAUTHORIZED, apiError, MSG_AUTH_REQUIRED);
    }
}
