...
La colección importable para Postman está en `docs/MDQR-Postman-QA.json` en la raíz del repositorio. Incluye todos los endpoints con ejemplos, variables de colección para QA y scripts que guardan el token automáticamente.
:::
...
```bash
TOKEN=$(curl -s \
  http://199.3.0.63:8180/realms/mdqr-admin/protocol/openid-connect/token \
