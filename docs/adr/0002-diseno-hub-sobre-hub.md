# ADR-0002 — Diseño del Hub de Interoperabilidad sobre la Base HUB
...
sobre el código heredado** del middleware `hub-interop` (group
`bo.com.sintesis.hub`).
...
3. ¿En qué orden se implementa para que cada fase sea desplegable sin romper la
   funcionalidad QR actual ni los contratos `hub-*` ya en uso?

La restricción transversal es de **decoupling diferido** (ADR-0001, nota final):
nombres heredados (`hub-*`, paquetes `bo.com.sintesis.hub.*`, tablas
`middleware_*` cuando existan, paths de Vault `hub-auth`/`hub-base`, nombres
de Consul `hubadminservice`/`hubbaseservice`, realms `hub-admin`/`hub-partner`)
...
- **hub-gateway**: dos cadenas de seguridad por `@Order` (partner Order=1 sobre
...
- **hub-ms-auth**: fachada sobre Keycloak Admin API (`keycloak-admin-client`),
...
- **hub-ms-base**: núcleo QR (decode ZXing + decrypt RSA/BouncyCastle), ciclo de
...
- **hub-audit-commons**: publisher fire-and-forget (buffer en memoria + retry en
...
|--------------------------|-----------------------------------------|------------|
| Edge / Gateway           | `hub-gateway`                          | **Ampliar**: añadir terminación mTLS, validación RFC 8705, rutas outbound. Conservar cadenas/filtros/rutas actuales. |
| Servicio de identidad    | `hub-ms-auth`                          | **Ampliar**: registro de partners, orquestación PKI Vault, gestión de suscripciones/scopes, sincronización a Redis (whitelist/rate-limit). |
| Núcleo del hub (inbound) | `hub-ms-base`                          | **Ampliar**: QR pasa a ser *un* producto inbound; añadir más rutas de negocio inbound bajo el mismo patrón. |
| Núcleo del hub (outbound)| `hub-ms-base` (fase 1) → módulo propio | **Ampliar primero, extraer después**: framework de adaptadores ACL dentro de ms-base; extraer a `hub-ms-outbound` si crece. |
| Librería de auditoría    | `hub-audit-commons`                    | **Ampliar**: canonicalización RFC 8785, hash SHA-256 de payloads, hash-chain, firma/cifrado Vault Transit. Aditivo sobre el `AuditEventDto` actual. |
| Servicio de facturación  | (nuevo) `hub-ms-billing`               | **Nuevo**: relay del outbox + medición + facturación. Inicia como relay dentro de ms-base, se extrae cuando haya señal de escalado independiente. |
| Frontend / portal        | `hub-frontend` (`apps/admin` + nueva `apps/portal`) | **Ampliar**: nueva app `portal` para partners; reutiliza el workspace y keycloak-angular. |
...
### hub-gateway — Edge / mTLS / Enrutamiento
...
`/oauth2/token`, valida JWT del realm `hub-partner` con `partnerJwtDecoder`) y
`adminSecurityChain` (Order=2, resto, realm `hub-admin`, JWT + OIDC browser).
...
`/realms/hub-partner/protocol/openid-connect/token`. Retry solo en GET.
...
3. **Nuevas rutas outbound.** Las apps internas llaman al hub para consumir
   ...
   gateway enruta a `lb://hubbaseservice` (fase 1) reescribiendo a la API outbound
   interna. Esta ruta vive bajo una cadena de seguridad **interna** (realm
   `hub-admin` o un realm/scope de servicio): las apps internas se autentican como
...
### hub-ms-auth — Identidad / Partners / PKI

**Qué existe hoy.** IAM y RBAC propios sobre Keycloak Admin API
(`keycloakAdminClient` con client_credentials del client `hubadminservice`).
...
de Keycloak. Se registra en Consul como `hubadminservice`.
...
1. **Registro de partners.** Nuevo agregado de dominio "partner" y su ciclo de
   ...
   crear/gestionar el client M2M del partner en el realm `hub-partner`
...
`/services/hubadminservice/admin/**`).**
...
### hub-ms-base — Núcleo inbound/outbound
...
2. **Framework de adaptadores outbound (ACL / Facade).** Nuevo paquete
   `bo.com.sintesis.hub.base.outbound` con:
...
### hub-audit-commons — Librería de auditoría extendida
...
del outbox (`hub_base`/schema `public`) y su volumen inicial es bajo. Aplicando
YAGNI y "monolito modular primero":

- **Fase inicial**: el relay vive **dentro de ms-base** como un componente
  `@Scheduled` (paquete `bo.com.sintesis.hub.base.billing`) que lee `outbox_event`
...
- **Extracción a `hub-ms-billing`** (módulo nuevo, patrón `hub-*`, Consul
  `hubbillingservice`): se hace cuando aparezca una señal real — facturación con
  ...
  manual, Consul, Vault namespace propio o `hub-base`).
...
**Nombre sugerido.** `hub-ms-billing` (cuando se extraiga). Mientras tanto,
...
opcionales en `AuditEventDto`. Sin hash-chain todavía. Módulo: `hub-audit-commons`.
...
en ms-auth. Módulos: `hub-audit-commons`, `hub-ms-auth`. Depende de Fase 0.
...
(`whitelist:ip:*`, `ratelimit:config:*`). Módulo: `hub-ms-auth`. Depende de Fase 1
...
holder-of-key en el client `unilink-api` de Keycloak. Módulo: `hub-gateway` +
...
`idempotency_key`, tabla de medición. Módulo: `hub-ms-base`. Depende de Fase 1
...
Nueva ruta `/outbound/v1/**` en el gateway. Módulos: `hub-ms-base`,
`hub-gateway`. Depende de Fases 1 y 4 (auditoría + outbox para outbound).
...
`apps/portal` en `hub-frontend`. Contratos OpenAPI versionados por producto.
Módulos: `hub-ms-base`, `hub-frontend`. Depende de Fase 3.

**Fase 7 (condicional) — Extraer `hub-ms-billing`.**
Solo si hay señal de escalado/equipo independiente. Mueve el relay de `…base.billing`
a un módulo `hub-*` propio con su Consul/Vault. Depende de Fase 4.

## Restricciones arquitectónicas

1. **No renombrar nada heredado.** Módulos `hub-*`, paquetes
   `bo.com.sintesis.hub.*`, realms `hub-admin`/`hub-partner`, nombres de Consul
   `hubadminservice`/`hubbaseservice`, namespaces de Vault `hub-auth`/`hub-base`,
   tablas `middleware_*`/`decode_log`/`audit_log`/`certificate*`. Los módulos nuevos
   siguen el patrón `hub-*`. El renombrado es decoupling posterior (ADR-0001).
...
- Diferir `hub-ms-billing` y el módulo outbound evita complejidad operativa
